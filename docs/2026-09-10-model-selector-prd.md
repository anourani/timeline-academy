...
| # | Decision | Status |
|---|---|---|
| D1 | UI is a plain dropdown. No slider, no thinking-effort control. | Locked in shape. **The primitive changed 12 Sep**: shipped on shadcn `Select`, now on the `DropdownMenu` behind `AccountMenu` — see Step 4. |
...
**Trigger.** A plain `<button>` through `<DropdownMenuTrigger asChild>` (12 Sep; it was a `SelectTrigger` before), with `className` in one of two variants. `type="button"` is mandatory — the tab is mounted inside the Create page's `<form>`, where the default `submit` type would make opening the menu fire a generation. The chevron is drawn here too, since `DropdownMenuTrigger` styles nothing. `chip` (the default, and what `ApiKeySection` renders): the `glassButtonClass` geometry — `rounded-[10px]`, `px-[11px] py-[6px]`, the blur and inset highlight — with `Sparkles` leading, the current model's `label`, default chevron trailing. `tab` (the Create page, 12 Sep): no fill, border or shadow of its own because the plate supplies them, `px-[16px] py-[8px]` at 12px/18px, the literal word "Model" in `text-text-tertiary` leading, the label in `#A770EC`, and a 12px chevron. The tab also drops the `aria-label` the chip carries — its visible text already names the setting, and an `aria-label` there would override the contents and hide the selected model from the accessible name.
...
**Content — rebuilt 12 Sep on `DropdownMenu`.** `select.tsx` is untouched stock shadcn, themed off `--popover`, which `index.css:21` defines as `217 33% 17%` — a blue-tinted slate. `ui/dropdown-menu.tsx` opens with a comment rejecting exactly that token, because every other floating surface in this product is the neutral `#171717` on `#404040`. So the model menu rendered blue a few hundred pixels from an account menu that rendered neutral. It now uses the same components: `DropdownMenuContent`, `DropdownMenuGroup`, `DropdownMenuLabel`, `DropdownMenuSeparator`, `DropdownMenuItem`, with no per-call restyling.

Two consequences worth recording. The **unlock row is now an ordinary item** — `UNLOCK_VALUE` and the interception it needed are deleted, because that sentinel only existed to smuggle an action row past a primitive that had to treat every row as a value. And model rows carry `role="menuitemradio"` + `aria-checked` as props, keeping the one-of-many relationship `Select`'s options had; this file's trimmed `dropdown-menu.tsx` exports no `RadioItem`, and adding one would give the shared component a left indicator gutter the account menu does not want.

`modal={false}`, like `AccountMenu` — though not for the reason its comment gives. `Modal.tsx` is a hand-rolled `createPortal`, not a Radix layer, so the stale-`pointer-events` trap does not fire (checked under `modal={true}`: the page stays clickable). What remains is that modal mode writes to `document.body.style` while the key modal's `lockScroll()` writes to the same object.

The original structure is unchanged: two groups with labels, in `MODEL_GROUP_ORDER` — **Anthropic first** (Alex, 10 Sep) — with labels from `PROVIDER_META[provider].label` so provider naming can't drift from the key modal. The key modal's fields stay in `PROVIDER_ORDER` (OpenAI first); the two orders differ on purpose and the constant's comment says so. Each `SelectItem`: label left, descriptor right in `text-text-secondary`. Locked items: `disabled`, `Lock` replacing the descriptor, opacity via the component's existing `data-[disabled]:opacity-50`. They stay visible.
...
- [ ] ~~`grep -rn "SelectGroup" src/` → `select.tsx` and `ModelSelector.tsx` only~~ **Restated 12 Sep**, the menu having moved off `Select`: `grep -rn "DropdownMenuGroup" src/` → `ui/dropdown-menu.tsx`, `AccountMenu.tsx` and `ModelSelector.tsx` only, and `grep -rn "SelectGroup" src/` no longer names `ModelSelector.tsx`. Still no custom dropdown.
- [ ] ~~`grep -n "#[0-9a-fA-F]\{6\}" src/components/Settings/ModelSelector.tsx` → zero.~~ **Retired 12 Sep** — it was already false. Three literals are there on purpose, and a blanket ban only invites a worse workaround: `#A770EC` for the tab's model name (deliberately not `CATEGORIES[0].color`, which is the same hex for an unrelated reason), and `#6b6e73` for the menu's trailing-decoration idiom, copied from `AccountMenu.tsx:116,122`. The neutrals cannot come from the shadcn tokens at all — that is the whole argument in `ui/dropdown-menu.tsx`'s header. Read as: no *new* hex beyond these without a reason written down beside it.
