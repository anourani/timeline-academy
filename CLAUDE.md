...
│                     #   byok-keys, set-byok-flag, delete-account,
│                     #   and _shared/
...
- `BYOK_ENCRYPTION_KEY` — base64 of 32 bytes (`openssl rand -base64 32`), the AES-256-GCM key `byok-keys` encrypts stored BYOK keys with. Missing or malformed → `byok-keys` answers 503 and the client silently falls back to browser-only keys, so the site keeps working. **Rotating it invalidates every stored key**: there is no key-id column and no second slot to decrypt against, so every BYOK user has to re-add theirs. Keep a copy in a password manager.
...
- **Server-funded AI requires sign-in.** Anonymous visitors hitting Generate get a gate offering email sign-in or their own OpenAI or Anthropic key. BYOK *requests* run browser-direct (`services/anthropicDirect.ts`, `services/openaiDirect.ts`) and never touch our servers — but BYOK *keys* do, for signed-in users. `getActiveModel()` in `services/userApiKey.ts` resolves the model and the key that pays for it together — and when exactly one key exists, that provider wins regardless of the stored model preference, which is what keeps the tier logic and the routing logic from disagreeing.
- **BYOK keys are an account feature for signed-in users.** They live in `public.user_byok_keys` as AES-256-GCM ciphertext (plus `user_byok_settings` for the chosen model), written and read only by the `byok-keys` Edge Function under the service role — both tables have RLS on with no policies and no `anon`/`authenticated` grants. `localStorage` remains the **synchronous cache** every reader uses (`getActiveModel`, the hooks, `lib/limits.ts`), so nothing downstream had to change. Sign-in pulls the account's keys down; a sync never deletes, it only fills gaps in either direction. Writes go local-first, except delete, which goes server-first — the other order lets a failed delete resurrect the key on the next sign-in. `SIGNED_OUT` clears the device and leaves the account's copy alone: signing out is leaving a device, not giving up the key.
- **`byok_enabled` is derived server-side now, from whether key rows exist.** It was previously reported up from localStorage, which meant signing in on a second browser — where there was no local key — pushed `{ enabled: false }` and silently demoted the account from BYOK limits to Free until the first browser re-saved. `set-byok-flag` stays deployed for cached old bundles; `byok-keys`'s `get` heals whatever they flip.
- **The model selector reads keys, never the server.** `src/constants/models.ts` is the single source of truth for the six models the dropdown offers and for every per-model difference in the request bodies (`params`); the adapters spread those and carry no per-model branches. The chosen model answers all three BYOK calls — classify, generate, and event descriptions — and is remembered in `timeline_byok_model` beside the keys it depends on. Our Edge Functions never accept or select a model: `generate-timeline` deliberately does not read a `model` field, and the Free tier stays pinned to Haiku/Sonnet server-side. So a model change is a site deploy only, never a function deploy.
