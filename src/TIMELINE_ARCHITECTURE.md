...
8a. [Chapters Strip](#chapters-strip)
...
26. [AI Generation Streaming](#ai-generation-streaming)
...
`findActiveChapter` applies `CHAPTER_ACTIVE_LEAD_MONTHS` (6) of lead to the left-most visible month, so a chapter's chip lights while its opening events are still out to the right rather than after the reader has scrolled past them. That lead is bounded twice, because an unbounded one lights the wrong chip immediately after a jump:

1. A start falling within `SCROLL_LEAD_IN_MONTHS` ahead of the left edge wins outright — that offset is exactly where a jump parks its chapter, so such a start is almost always the chip just clicked. Where two land in that window (a chapter shorter than the lead-in wedged in front of the target), the **later** one wins: it is both the likelier target and the one whose events fill the screen.
2. Otherwise the lead runs its full six months, capped at the next chapter's start, so it can pull that chapter forward without ever clearing a short one entirely.

Without these, a jump left the probe four months *into* the target chapter, so anything shorter than five months lit its successor's chip — which reads as the jump having missed. Where every chapter is longer than the lead, both bounds are inert and the result is identical to the plain `monthIndex + 6`.
...
`suppressStackAnimation` disables the FLIP entirely while a generation is streaming. Events arrive in date order so reshuffles are rare, but sliding one that only just appeared reads as a glitch rather than as the considered reflow it is after a drag. `revealOnMount` plays the arrival animation instead — see [AI Generation Streaming](#ai-generation-streaming).
...
### Why `target` and the rAF id live in refs

`wheelTargetRef` and `wheelRafRef` are component refs rather than locals of the effect, so that `scrollToTarget` (§16) can stop the loop. Assigning `scrollLeft` aborts an in-flight smooth scroll, so the two are not merely racing — whichever ran last won. The loop stays alive for roughly 450–900ms after the last wheel tick (0.18 decays the remaining distance by ×0.82 per frame), and trackpad inertia re-arms it for a second or two beyond that. Any jump issued in that window was silently overwritten and dragged back to the wheel's own target, which read as a dead chip — and only ever right after scrolling, since the one way to be sitting exactly at a chapter's start is to have clicked a chip to get there.

`wheelSuppressUntilRef` is the other half: `scrollToTarget` sets a deadline `WHEEL_SUPPRESS_MS` (150, `constants/timeline.ts`) out, and wheel events arriving before it are `preventDefault`ed and dropped, each one pushing the deadline forward again. That rolling window is what distinguishes inertia — an unbroken ~16ms stream carried over from the gesture that preceded the click — from a deliberate scroll, which arrives after a pause and gets through. A fixed window cannot do this: inertia outlasts any window short enough not to swallow real input.
...
**`scrollToTarget` cancels the wheel lerp before it scrolls**, and arms the suppression window described in §15 — otherwise the loop overwrites the jump frame by frame. It deliberately does *not* resync `wheelTargetRef` to the new destination: leaving the loop idle is exactly what lets the next wheel tick pick up the live mid-flight position, so a deliberate scroll still interrupts a jump cleanly rather than snapping back to where it started.

Native `scrollTo` rather than routing the jump through the lerp, because the platform already gets three things right that the loop would not: it re-clamps to `scrollWidth - clientWidth` continuously (which matters while the grid's `min-width` is still transitioning after a scale change), it eases *in* (the lerp's first frame moves 18% of the distance — a visible teleport on a long jump), and it honours `prefers-reduced-motion`.

### Entry points

1. **`pendingScrollTarget` prop.** A `useEffect` runs `scrollToTarget(pendingScrollTarget)` inside `requestAnimationFrame` and then calls `onScrollComplete?.()` so the parent can clear its pending state. `onScrollComplete` fires when the scroll *starts*, not when it finishes — the name is a misnomer, but deferring it would strand `pendingScrollTarget` whenever the target equals the current position, since no scroll means no `scrollend`. Because both hosts pass it as an inline arrow and `Timeline` is not memoised, the effect re-runs on every render while the target is set; it cancels its previous frame so that stays one `scrollToTarget` per target rather than one per render. `App.tsx` sets `pendingScrollTarget` to the earliest event's `startDate` (bare string, so it centres) in five places — see "Auto-scroll on initial load" below.
...
`getTimelineRange(spans, rangeOverride?)` in `src/utils/dateUtils.ts` decides which months to render. `spans` is anything with `startDate`/`endDate` — normally the events.

```
0. If a rangeOverride is given, seed the year pool with its startYear and
   endYear. It WIDENS the pool rather than replacing it — see below.
