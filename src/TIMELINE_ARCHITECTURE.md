...
Chapters themselves are `{ id, label, startDate, endDate }` on `timelines.chapters` (jsonb), generated by the AI pass. `src/utils/chapters.ts` holds the pure logic: `normalizeChapters` (validate, sort, assign ids), `findChapterAtMonth`, `chapterProgress`, `countEventsInChapter`. A timeline with no chapters renders nothing.
